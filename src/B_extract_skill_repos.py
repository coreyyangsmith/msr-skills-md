...
def main(argv: List[str]) -> int:
    ...
    repos_to_scan = [r for r in repos if r.repo not in already and r.repo not in blacklist]
    if blacklist:
        log.info("Blacklist: skipping %d blacklisted repositories.", sum(1 for r in repos if r.repo in blacklist))
