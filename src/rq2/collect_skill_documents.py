...
HEADING_H1_RE = re.compile(r"^#\s+\S")
HEADING_H2_RE = re.compile(r"^##\s+\S")
HEADING_H3_RE = re.compile(r"^###\s+\S")
...
FILE_PATH_PATTERN = re.compile(
    r"\b(?:\.{0,2}/)?[\w\-./]+\.(?:py|js|ts|sh|yaml|yml|json|md|txt)\b"
)
...
def collect_markdown_structure(text: str) -> dict[str, Any]:
    ...
    heading_h1_count = sum(1 for line in lines if HEADING_H1_RE.match(line))
    heading_h2_count = sum(1 for line in lines if HEADING_H2_RE.match(line))
    heading_h3_count = sum(1 for line in lines if HEADING_H3_RE.match(line))
...
def extract_commands_from_code_blocks(text: str) -> list[str]:
...
def extract_inline_commands(text: str) -> list[str]:
    return [value.strip() for value in INLINE_CODE_PATTERN.findall(text) if value.strip()]


def extract_file_paths(text: str) -> list[str]:
    return sorted(set(FILE_PATH_PATTERN.findall(text)))
...
def classify_reference(path: str) -> str:
    ...
    if path_lower.endswith((".sh", ".py", ".js", ".ts")):
        return "script"
    if path_lower.endswith(".md"):
        return "documentation"
    if path_lower.endswith((".json", ".yaml", ".yml", ".txt")):
        return "asset"
    return "other"
...
def _build_document(path: Path, raw_data_dir: Path) -> dict[str, Any] | None:
    ...
    code_block_commands = extract_commands_from_code_blocks(text)
    inline_commands = extract_inline_commands(text)
    all_commands = code_block_commands + inline_commands
    ...
    return {
        ...
        "structure": structure,
        "commands": {
            "from_code_blocks": code_block_commands,
            "from_inline_code": inline_commands,
            "all": all_commands,
        },
        "references": {
            "items": references,
...
def compute_stats(documents: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    docs_with_commands = 0
    for doc in documents:
        commands = list(doc.get("commands", {}).get("all", []))
        if commands:
    ...
    stats: dict[str, Any] = {
        ...
        "commands": {
            "total_extracted_commands": sum(command_counter.values()),
            "unique_commands": len(command_counter),
            "docs_with_commands": docs_with_commands,
            "docs_without_commands": total_docs - docs_with_commands,
            "command_usage_rate": (docs_with_commands / total_docs) if total_docs else 0.0,
            "top_50_commands": command_counter.most_common(50),
        },
        "references": {
