"""
Tests for B_extract_skill_repos.py
...
from B_extract_skill_repos import (
    OUTPUT_COLUMNS,
    RepoSource,
    ScanResult,
    append_result,
...
class TestScanOneRepo(unittest.TestCase):
    ...
    def test_found_via_contents_api(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(), 200, "")), \
             mock.patch("B_extract_skill_repos.try_contents_path", return_value=(True, self._file_data(), 200, "")):
    ...
    def test_not_found_contents_api_returns_found_false(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(), 200, "")), \
             mock.patch("B_extract_skill_repos.try_contents_path", return_value=(False, {}, 404, "")):
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
                enable_code_search=False, min_stars=0, allow_forks=True, allow_archived=True,
            )
    ...
    def test_repo_metadata_404_sets_invalid_repo_error(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(None, 404, "Not Found")):
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
                enable_code_search=False, min_stars=0, allow_forks=True, allow_archived=True,
            )
    ...
    def test_filtered_by_min_stars(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(stars=3), 200, "")): 
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
    ...
    def test_filtered_fork(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(fork=True), 200, "")):
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
    ...
    def test_filtered_archived(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(archived=True), 200, "")):
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
    ...
    def test_code_search_fallback_finds_file_in_subdir(self):
        ...
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(), 200, "")), \
             mock.patch("B_extract_skill_repos.try_contents_path", return_value=(False, {}, 404, "")), \
             mock.patch("B_extract_skill_repos.try_code_search", return_value=(True, search_item, 200, "")):
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
    ...
    def test_code_search_not_called_when_disabled(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(), 200, "")), \
             mock.patch("B_extract_skill_repos.try_contents_path", return_value=(False, {}, 404, "")), \
             mock.patch("B_extract_skill_repos.try_code_search") as mock_search:
            scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
                enable_code_search=False, min_stars=0, allow_forks=True, allow_archived=True,
            )
    ...
    def test_auth_error_on_contents_stops_scan_early(self):
        ...
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(), 200, "")), \
             mock.patch("B_extract_skill_repos.try_contents_path", return_value=(False, {}, 401, "Unauthorized")), \
             mock.patch("B_extract_skill_repos.try_code_search") as mock_search:
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
                enable_code_search=True, min_stars=0, allow_forks=True, allow_archived=True,
    ...
    def test_rate_limit_on_contents_falls_through_to_code_search(self):
        ...
        """
        with mock.patch("B_extract_skill_repos.get_repo_metadata", return_value=(_make_meta(), 200, "")), \
             mock.patch("B_extract_skill_repos.try_contents_path", return_value=(False, {}, 403, "rate limited")), \
             mock.patch("B_extract_skill_repos.try_code_search", return_value=(False, None, 200, "")) as mock_search:
            result = scan_one_repo(
                self._gh(), self._src(), "SKILL.md", ["/SKILL.md"],
                enable_code_search=True, min_stars=0, allow_forks=True, allow_archived=True,
            )
    ...
    def test_metadata_populated_from_repo_meta(self):
        with mock.patch("B_extract_skill_repos.get_repo_metadata",
...
class TestCheckRateLimit(unittest.TestCase):
    ...
    def test_logs_remaining_counts(self):
        ...
        with self.assertLogs("B_extract_skill_repos", level="INFO") as cm:
            check_rate_limit(gh)
    ...
    def test_returns_empty_dict_on_non_200(self):
        ...
        with self.assertLogs("B_extract_skill_repos", level="WARNING"):
            result = check_rate_limit(gh)
    ...
    def test_logs_warning_on_non_200(self):
        gh = self._mock_gh()
        gh.request_json.return_value = (403, {}, "Forbidden")
        with self.assertLogs("B_extract_skill_repos", level="WARNING") as cm:
            check_rate_limit(gh)
