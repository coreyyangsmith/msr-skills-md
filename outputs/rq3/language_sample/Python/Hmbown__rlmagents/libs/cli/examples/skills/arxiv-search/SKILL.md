...
If running `rlmagents` from a virtual environment:
```bash
...
Replace `[YOUR_SKILLS_DIR]` with the absolute skills directory path from your system prompt (e.g., `~/.rlmagents/agent/skills` or the full absolute path).
...
```bash
.venv/bin/python ~/.rlmagents/agent/skills/arxiv-search/arxiv_search.py "deep learning drug discovery" --max-papers 5
```

Search for computational biology papers:
```bash
.venv/bin/python ~/.rlmagents/agent/skills/arxiv-search/arxiv_search.py "protein folding prediction"
```

Search for bioinformatics methods:
```bash
.venv/bin/python ~/.rlmagents/agent/skills/arxiv-search/arxiv_search.py "genome assembly algorithms"
```
...
If running `rlmagents` from a virtual environment (recommended), use the venv's Python:
```bash
